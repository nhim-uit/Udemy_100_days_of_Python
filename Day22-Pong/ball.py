...
class Ball(Turtle):
    ...
    def detect_paddle(self, paddle: Paddle):
        if self.distance(paddle.get_paddle()) < 2 * PIXEL and self.xcor() > WALL_WIDTH - 2 * PIXEL \
                or self.distance(paddle.get_paddle()) < 2 * PIXEL and self.xcor() < - WALL_WIDTH + 2 * PIXEL:
            self.bounce_x()
